...
// ---- runtime helpers ------------------------------------------------------
//
// The `RuntimeFn` implementations: semantics the compiler needs but that
// are not opcodes (mirroring V8's split — only frame-coupled and
// control-flow operations get opcodes). Invoked via `CallRuntime` at
// indices 0..RuntimeFn::COUNT. User-code throws surface as the exception
// sentinel return value; VM errors surface as `Err`.
