...
use crate::{
    AccessorPair, Context, Convert, DenseString, FixedArray, GcSlice, Handle, Heap, HeapRef, Key,
    LoadOutcome, Lookup, Object, ObjectSlotsInit, PropertyDescriptor, SlotName, Smi, StoreOutcome,
    StoreSemantics, StringData, Symbol, Tagged, Value, VmError, classify_key, function_kind_of,
    home_proto, load_outcome, private_find, runtime::Coercion, super_constructor,
...
fn delete_property(
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        let ok = if nctx
            .heap()
            .no_gc(|heap| crate::proxy::is_proxy(heap, unsafe { target.assume_valid(heap) }))
        {
            let (vm, heap, state) = nctx.split();
            // Safety: fresh rooted-slot word (re-read above) plus a fresh
            // coercion result, both consumed by the trap call.
            match crate::proxy::delete(
                vm,
                heap,
                state,
                unsafe { Tagged::<Value>::from_value_unchecked(target) },
                unsafe { Tagged::<Value>::from_value_unchecked(key) },
            )? {
                // Safety: fresh root-slot word read for the immediate return.
                Coercion::Threw => return Ok(unsafe { heap.known().exception.read_unchecked() }),
                Coercion::Value(v) => {
                    heap.no_gc(|heap| Convert::is_truthy(heap, unsafe { v.assume_valid(heap) }))
                }
            }
        } else {
...
fn delete_property_core(
    ...
    if nctx
        .heap()
        .no_gc(|heap| Convert::is_primitive(heap, unsafe { target.assume_valid(heap) }))
    {
        let owned = nctx.heap().no_gc(|heap| {
            string_exotic_own(
                heap,
    ...
    nctx.handle_scope(|nctx, scope| {
        let receiver = scope
            .cast::<Object>(
                ...
                unsafe { target.assume_valid(&*nctx.heap()) },
            )
...
fn for_in_enumerate(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    if nullish {
        // Safety: fresh root-slot word read for the immediate return.
        return Ok(unsafe { nctx.heap().known().undefined.read_unchecked() });
    }
    let level = nctx
        .heap()
        .no_gc(|heap| for_in_initial_level(heap, unsafe { subject.assume_valid(heap) }));
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        let enumerator = heap.new_object(
            &scope,
            map,
            scope.stage_words(&[level_word, keys_word, Smi::new(0).encode(), empty_word]),
        );
...
fn for_in_initial_level(heap: &Heap, subject: Tagged<'_, Value>) -> Option<Value> {
    ...
    let ctor_name = if Smi::decode(subject.erase()).is_some() {
        "Number"
    } else if subject.get_as::<crate::Float>().is_some() {
        "Number"
    ...
    // Safety: fresh root-slot words read for the pointer-keyed names.
    let name = SlotName::from_value(match ctor_name {
        "Number" => unsafe { strings.number_ctor.read_unchecked() },
        "Boolean" => unsafe { strings.boolean_ctor.read_unchecked() },
        _ => unsafe { strings.symbol_ctor.read_unchecked() },
    });
    // Safety: root-slot / walk words, still fresh (no allocation since).
    let ctor =
        match crate::lookup::load_outcome(heap, unsafe { global.assume_valid(heap) }, name).ok()? {
            crate::LoadOutcome::Value(v) if v.is_strong_ptr() => v.erase(),
            _ => return None,
        };
    // Safety: fresh root-slot word read for the pointer-keyed name.
    let proto_name =
        SlotName::from_value(unsafe { heap.known().strings.prototype.read_unchecked() });
    match crate::lookup::load_outcome(heap, unsafe { ctor.assume_valid(heap) }, proto_name).ok()? {
        crate::LoadOutcome::Value(p) if p.is_strong_ptr() => Some(p.erase()),
        _ => None,
    }
}
...
fn for_in_level_keys(
    ...
    let (mut indices, names) = heap.no_gc(|heap| {
        // Safety: caller-supplied word, fresh at entry.
        ...
        for d in obj.map_ref(heap).descriptors() {
            ...
            // Safety: pointer-keyed name word re-anchored for the check.
            ...
            let index = unsafe { name.tagged(heap) }
                .erase_type()
                .get_as::<DenseString>()
                .and_then(|s| crate::lookup::canonical_index(s.as_ref().data(heap)))
...
fn for_in_next(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        loop {
            ...
            let candidate = heap.no_gc(|heap| -> Result<Option<Value>, VmError> {
                let Some(obj) =
            ...
            let Some(key) = candidate else {
                ...
                let level = heap.no_gc(|heap| -> Result<Value, VmError> {
                    let Some(obj) =
                        unsafe { enumerator.read_unchecked().assume_valid(heap) }.as_heap_object()
                    else {
                        return Err(VmError::Type);
                ...
                let Some(proto) = for_in_next_level(vm, heap, level)? else {
                    ...
                    return Ok(unsafe { heap.known().undefined.read_unchecked() });
                };
            ...
            let seen = heap.no_gc(|heap| -> Result<bool, VmError> {
                let Some(obj) =
                    unsafe { enumerator.read_unchecked().assume_valid(heap) }.as_heap_object()
                else {
                    return Err(VmError::Type);
                ...
                Ok(visited.as_slice().iter().any(|s| s.inner() == key_word))
            })?;
            if seen {
                continue;
            }
            ...
            {
                let visited = heap.no_gc(|heap| -> Result<Vec<Value>, VmError> {
                    let Some(obj) =
                        unsafe { enumerator.read_unchecked().assume_valid(heap) }.as_heap_object()
                    else {
                        return Err(VmError::Type);
                ...
                let visited =
                    heap.allocate_handle::<FixedArray>(scope.stage_words(&visited), &scope);
                heap.no_gc(|heap| -> Result<(), VmError> {
                    let Some(obj) =
                        unsafe { enumerator.read_unchecked().assume_valid(heap) }.as_heap_object()
                    else {
            ...
            return Ok(unsafe { key.read_unchecked() });
        }
    })
}
...
fn for_in_next_level(_vm: &VM, heap: &mut Heap, level: Value) -> Result<Option<Value>, VmError> {
    heap.no_gc(|heap| {
            ...
            .is_some()
        {
            ...
            let global = heap.known().global_object.as_tagged(heap).erase();
            let string_name =
                SlotName::from_value(unsafe { heap.known().strings.string.read_unchecked() });
            let Some(string_ctor) = crate::lookup::load_outcome(
                heap,
            ...
            .ok()
            .and_then(|o| match o {
                crate::LoadOutcome::Value(v) => Some(v.erase()),
            ...
            let proto_name =
                SlotName::from_value(unsafe { heap.known().strings.prototype.read_unchecked() });
            let proto = crate::lookup::load_outcome(
                heap,
...
fn for_in_own_state(heap: &Heap, level: Value, key: Value) -> Option<bool> {
    // Safety: caller-supplied words, fresh at entry.
    match crate::lookup::classify_key(heap, unsafe { key.assume_valid(heap) }).ok()? {
        crate::Key::Element(i) => {
            // Safety: caller-supplied word, fresh at entry.
...
fn get_iterator(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    nctx.handle_scope(|nctx, scope| {
        ...
        let undefined_or_null = nctx.heap().no_gc(|heap| {
            method == heap.known().undefined.as_tagged(heap).erase()
                || method == heap.known().null.as_tagged(heap).erase()
        });
        if undefined_or_null || !crate::runtime::Runtime::is_callable(nctx.heap(), method) {
        ...
        nctx.call(
            // Safety: fresh lookup result, consumed by the call.
            unsafe { Tagged::<Value>::from_value_unchecked(method) },
            scope.stage_words(&[obj_word]),
        )
    })
}
...
fn iterator_next(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    nctx.handle_scope(|nctx, scope| {
        ...
        let (vm, heap, state) = nctx.split();
        // Safety: fresh root-slot word read for the pointer-keyed name.
        let next_name = SlotName::from_value(unsafe { heap.known().strings.next.read_unchecked() });
        let iter_word = iter.as_tagged(&*heap).erase();
        let next =
            crate::runtime::Runtime::get_property(vm, heap, state, iter_word, next_name.value())?;
        let next = match next {
            // Safety: fresh root-slot word read for the immediate return.
            Coercion::Threw => {
                return Ok(unsafe { nctx.heap().known().exception.read_unchecked() });
        ...
        let result = nctx.call(
            // Safety: fresh lookup result, consumed by the call.
            unsafe { Tagged::<Value>::from_value_unchecked(next) },
            scope.stage_words(&[iter_word]),
        )?;
        if nctx
            .heap()
...
fn iterator_done(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    let (vm, heap, state) = nctx.split();
    // Safety: fresh root-slot word read for the pointer-keyed name.
    let done_name = SlotName::from_value(unsafe { heap.known().strings.done.read_unchecked() });
    let done = crate::runtime::Runtime::get_property(vm, heap, state, result, done_name.value())?;
    match done {
        // Safety: fresh root-slot word read for the immediate return.
        Coercion::Threw => Ok(unsafe { nctx.heap().known().exception.read_unchecked() }),
        Coercion::Value(v) => {
            let truthy = nctx
                .heap()
                .no_gc(|heap| Convert::is_truthy(heap, unsafe { v.assume_valid(heap) }));
            Ok(nctx
                .heap()
                .no_gc(|heap| Convert::boolean(heap, truthy).erase()))
        }
    }
}
...
fn iterator_value(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    let (vm, heap, state) = nctx.split();
    // Safety: fresh root-slot word read for the pointer-keyed name.
    let value_name = SlotName::from_value(unsafe { heap.known().strings.value.read_unchecked() });
    match crate::runtime::Runtime::get_property(vm, heap, state, result, value_name.value())? {
        // Safety: fresh root-slot word read for the immediate return.
        Coercion::Threw => Ok(unsafe { nctx.heap().known().exception.read_unchecked() }),
...
fn copy_data_properties(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    if nctx
        .heap()
        .no_gc(|heap| Convert::is_primitive(heap, unsafe { source.assume_valid(heap) }))
    {
        return Ok(target);
    }
    let (threw, target_out) =
        nctx.handle_scope(|nctx, scope| -> Result<(bool, Value), VmError> {
            ...
            let target_handle =
                scope.handle(unsafe { Tagged::<Value>::from_value_unchecked(target) });
            let source_handle =
                scope.handle(unsafe { Tagged::<Value>::from_value_unchecked(source) });
            ...
            for key in keys {
                ...
                let exists = heap.no_gc(|heap| {
                    !matches!(
                        unsafe { target_handle.read_unchecked().assume_valid(heap) }
                            .lookup(heap, SlotName::from_value(key_word)),
                        Lookup::NotFound
                    )
                });
                if exists {
                ...
                let target_word = target_handle.as_tagged(&*heap).erase();
                let key_word = key.as_tagged(&*heap).erase();
...
fn create_private_name(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        let sym = Symbol::new(nctx.heap(), &scope, desc.as_bytes());
        // Safety: fresh rooted-slot word, returned without an
        // intervening allocation.
...
fn private_get(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    nctx.heap().no_gc(|heap| {
        let obj = args.get(heap, 0).ok_or(VmError::Arity)?;
        let key = args.get(heap, 1).ok_or(VmError::Arity)?;
        match private_find(heap, obj, key).map(|s| s.inner()) {
            Some(v) => Ok(v),
...
fn private_set(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    let value = nctx.heap().no_gc(|heap| -> Result<Value, VmError> {
        let obj = args.get(heap, 0).ok_or(VmError::Arity)?;
        let key = args.get(heap, 1).ok_or(VmError::Arity)?;
...
fn private_in(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    let has = nctx.heap().no_gc(|heap| {
        let key = args.get(heap, 0).ok_or(VmError::Arity)?;
...
fn set_class_fields(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    nctx.heap().no_gc(|heap| {
        let ctor = args.get(heap, 0).ok_or(VmError::Arity)?;
        let fields = args.get(heap, 1).ok_or(VmError::Arity)?;
        let mut ok = false;
...
fn init_instance_fields(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    let count = nctx.heap().no_gc(|heap| {
    ...
    let threw_or_failed = nctx.handle_scope(|nctx, scope| -> Result<bool, VmError> {
        ...
        let exception = unsafe { nctx.heap().known().exception.read_unchecked() };
        let mut i = 0;
        while i + 1 < count {
            let raw_key = nctx.heap().no_gc(|heap| {
            ...
            let value = nctx.call(
                ...
                scope.stage_words(&[instance_word]),
            )?;
            if value == exception {
                return Ok(true);
            }
            ...
            let defined = {
                let heap = nctx.heap();
                let instance_word = instance.as_tagged(&*heap).erase();
                Object::define_own_property_values(
                    heap,
                    &scope,
                    // Safety: fresh rooted-slot words; the define roots
                    // its inputs.
                    instance_word,
                    SlotName::from_value(unsafe { key.read_unchecked() }),
                    PropertyDescriptor::Data {
                        value: unsafe { value.read_unchecked() },
                        writable: true,
                        enumerable: true,
                        configurable: true,
                    },
...
fn apply_store_outcome(
    ...
    match outcome {
        StoreOutcome::Transition { receiver, name } => {
            nctx.handle_scope(|nctx, scope| {
                Object::add_own_property_values(
                    nctx.heap(),
                    &scope,
                    // Safety: fresh walk words; the define roots its inputs.
                    receiver,
                    name,
        ...
        StoreOutcome::CallSetter { setter } => {
            ...
            let exception = unsafe { nctx.heap().known().exception.read_unchecked() };
            // Safety: fresh walk word, consumed by the call.
            let result = nctx.handle_scope(|nctx, scope| {
                nctx.call(
                    unsafe { Tagged::<Value>::from_value_unchecked(setter) },
                    // Safety: caller-supplied words, staged for the call.
                    scope.stage_words(&[receiver, value]),
                )
            })?;
            Ok(result == exception)
        }
...
fn get_property_lenient(
    ...
    nctx.handle_scope(|nctx, scope| {
        nctx.call(
            // Safety: fresh walk word, consumed by the call.
            unsafe { Tagged::<Value>::from_value_unchecked(getter) },
...
fn set_function_name(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        // Safety: fresh coercion result, consumed before any allocation.
        let key = unsafe { Tagged::<Value>::from_value_unchecked(key) };
        ...
        let defined = {
            ...
            let explicit = heap.no_gc(|heap| {
                // Safety: fresh root-slot word read for the pointer-keyed name.
                let plain = SlotName::from_value(unsafe { name_key.read_unchecked() });
                match fn_obj.heap_ref(heap).as_ref().lookup(heap, plain) {
            ...
            if explicit {
                ...
                Object::define_own_property(
                    ...
                    name_key,
                    PropertyDescriptor::Data {
                        // Safety: fresh rooted-slot word; the define roots
                        // its inputs.
                        value: name,
                        writable: false,
                        enumerable: false,
...
fn install_accessor(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        let (name, desc) = heap.no_gc(|heap| -> Result<_, VmError> {
            ...
            let name = match classify_key(heap, unsafe { key.assume_valid(heap) })? {
                Key::Element(i) => SlotName::from(Tagged::from_smi(Smi::new(i as i64))),
                Key::Name(name) => name,
            };
            // existing own accessor half, if any (own descriptors only)
            // Safety: fresh root-slot word for the oddball singleton.
            ...
            if let Some(obj) = target.as_tagged(heap).as_heap_object() {
                for d in obj.as_ref().header.map.heap_ref(heap).descriptors() {
                    if d.name() == name && d.flags().is_accessor() {
                        // Safety: fresh slot word, consumed here.
                        let pair =
                            unsafe { Tagged::<Value>::from_value_unchecked(d.value.inner()) }
                                .get_as::<AccessorPair>()
                                .map(|p| (p.as_ref().get.inner(), p.as_ref().set.inner()))
        ...
        // Safety: fresh rooted-slot words; the define roots its inputs.
        let target_word = target.as_tagged(&*heap).erase();
        let defined = Object::define_own_property_values(heap, &scope, target_word, name, desc)?;
        if !defined {
            return Err(VmError::Type);
        }
...
fn define_own_property(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        let value = value.as_tagged(&*heap).erase();
        // proxies run their `defineProperty` trap (ES 20.2.5.6); define
        // sites are strict-mode: a rejected define throws
        if heap.no_gc(|heap| crate::proxy::is_proxy(heap, unsafe { receiver.assume_valid(heap) })) {
            let partial = heap.no_gc(|heap| -> Result<crate::PartialDescriptor, VmError> {
                let enumerable = flags & bytecode::PropertyFlags::DontEnum.bits() == 0;
                let configurable = flags & bytecode::PropertyFlags::DontDelete.bits() == 0;
                if flags & bytecode::PropertyFlags::Accessor.bits() != 0 {
                    // Safety: fresh handle word, consumed here.
                    let pair = unsafe { value.assume_valid(heap) }
                        .get_as::<AccessorPair>()
                        .ok_or(VmError::Type)?;
            ...
            // Safety: fresh handle words, consumed by the trap call.
            return match crate::proxy::define_internal(
                vm,
                heap,
                state,
        ...
        let (name, desc) = heap.no_gc(|heap| -> Result<_, VmError> {
            if unsafe { receiver.assume_valid(heap) }
                .as_heap_object()
                .is_none()
            {
                return Err(VmError::Type);
        ...
        // Safety: fresh handle words; the define roots its inputs.
...
fn set_prototype(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    // Safety: fresh argument words, consumed below.
    let (obj, proto) = nctx.heap().no_gc(|heap| {
        Ok((
            args.get(heap, 0).ok_or(VmError::Arity)?.erase(),
            args.get(heap, 1).ok_or(VmError::Arity)?.erase(),
        ))
...
fn construct_super_construct(
    ...
    nctx.handle_scope(|nctx, scope| {
        ...
        let the_hole = unsafe { nctx.heap().known().the_hole.read_unchecked() };
        if new_target_v == undefined || new_target_v == the_hole {
            // not inside a [[Construct]]: reachable via an arrow that escaped
            // the constructor
            return Err(VmError::Type);
        }
        let derived = nctx.heap().no_gc(|heap| {
            // Safety: caller-supplied word, fresh at entry.
            function_kind_of(heap, unsafe { callee_v.assume_valid(heap) })
        ...
        let (receiver, allocated) = if derived {
            ...
            match crate::runtime::Runtime::create_construct_receiver(vm, heap, state, new_target) {
                ...
                // Safety: fresh call result, rooted below.
                Ok(Some(r)) => (
                    scope.handle(unsafe { Tagged::<Value>::from_value_unchecked(r) }),
                    true,
                ),
                // Safety: fresh root-slot word read for the immediate return.
                Ok(None) => return Ok(unsafe { nctx.heap().known().exception.read_unchecked() }),
...
fn construct_super_via(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    let callee = nctx.heap().no_gc(|heap| {
...
fn load_dynamic_name(nctx: &mut NativeContext<'_>, args: GcSlice<'_>) -> Result<Value, VmError> {
    ...
    match found {
        Some(v) if !is_the_hole(nctx, v) => Ok(v),
        ...
        None => {
            ...
            let outcome = nctx.heap().no_gc(|heap| {
                ...
                unsafe { global.assume_valid(heap) }.store_lookup(
                    heap,
                    // Safety: fresh root-slot word read for the name.
                    SlotName::from_value(name),
